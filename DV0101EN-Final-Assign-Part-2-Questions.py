...
app.layout = html.Div([
    ...
    html.H1(
        ...
        style={'textAlign': 'left', 'color': '#503D36', 'fontSize': '24px'}  # Include style for title
    ),
...
# Define the callback function to update the disabled input container for year dropdown based on selected statistics
@app.callback(
...
# Define the callback function to update the output container based on the selected year and statistics type
@app.callback(
    Output(component_id='output-container', component_property='children'),
    [Input(component_id='select-year', component_property='value'), Input(component_id='select-statistics', component_property='value')]
)
def update_output_container(selected_year, selected_statistics):
    if selected_statistics == 'Recession Period Statistics':
        ...
        unemp_rate = recession_data.groupby(['unemployment_rate', 'Vehicle_Type']).agg({'Automobile_Sales': 'mean'}).reset_index()
        R_chart4 = dcc.Graph(
            figure=px.bar(unemp_rate, x='unemployment_rate', y='Automobile_Sales',
        ...
        Y_chart1 = dcc.Graph(
            figure=px.line(yas, x='Year', y='Automobile_Sales', title='Automobile Sales for the Year')
        )
        
        # Plot 2: Total Monthly Automobile sales using line chart
        mas = yearly_data.groupby('Month')['Automobile_Sales'].sum().reset_index()
        Y_chart2 = dcc.Graph(
            figure=px.line(mas, x='Month', y='Automobile_Sales', title='Total Automobile Sales by Month')
        )
